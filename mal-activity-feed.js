...
(async function () {
	...
	if (location.href.includes('https://myanimelist.net/forum/?clubid=92324') || location.href.includes('https://myanimelist.net/clubs.php?cid=92324')) {
	...
	async function fetchTopicMessages(topicLink, parser, repliesActions, repliesInner) {
		...
		messages.forEach((message, index) => {
			if (index === 0) {
				...
				messageDiv.prepend(LinkButton, editButton);
			...
			if (index > 0) {
				...
				likeButton.addEventListener('click', async function () {
					if (!likeLoading) {
						...
						const whoLiked = likes.filter(item => item.body && item.body.includes(`likedReply/${messageId}/`));
						const whoLikedDiv = $(this).parent().children('.feed-like-wrap-users');
						const likeWrapDiv = $(this).parent();
